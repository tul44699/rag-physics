...
if CLIENT_DIST.is_dir():
    app.mount(
        "/assets", StaticFiles(directory=CLIENT_DIST / "assets"), name="client_assets"
    )
...
def register(payload: RegisterRequest, db: Session = Depends(get_db)):
    ...
    return AuthResponse(
        token=token, user_id=user.id, email=user.email, display_name=user.display_name
    )
...
def get_textbook_chunks(
    ...
    if chapter:
        q = q.filter(TextChunk.chapter == chapter)
    if page is not None:
        q = q.filter(TextChunk.page_start <= page, TextChunk.page_end >= page)
...
def get_textbook_anchors(
    ...
    chunks = (
        db.query(TextChunk)
        .filter(TextChunk.textbook_id == textbook_id, TextChunk.chunk_type != None)
        .all()
    )
    ...
    for eq in eqs:
        anchors.append(
            Anchor(
                ...
                page=eq.page_start,
                chapter=eq.chapter,
                label=eq.plain_text[:80],
                snippet=eq.plain_text,
            )
        )
    return anchors
...
def generate_formula_sheet(
    ...
    q = db.query(Equation)
    if textbook_ids:
        q = q.filter(Equation.textbook_id.in_(textbook_ids))
    if chapter:
        q = q.filter(Equation.chapter.ilike(f"%{chapter}%"))
    if page_start is not None:
        q = q.filter(Equation.page_start >= page_start)
    if page_end is not None:
        q = q.filter(Equation.page_start <= page_end)
    ...
    for eq in q.order_by(Equation.page_start).all():
        ...
        if ch.startswith(_NOISE_PREFIXES):
            continue
        ...
        chapters[ch][sec].append(
            FormulaSheetItem(
                latex=eq.latex,
                plain_text=eq.plain_text,
                variables=eq.variables,
                page_start=eq.page_start,
                section=eq.section,
            )
        )

    return FormulaSheetResponse(
        sections=chapters, chapter_order=chapter_order, section_order=section_order
    )
        ...
        "relations": {
            ...
            "items": {
                ...
                "properties": {
                    ...
                    "type": {
                        "type": "string",
                        "enum": ["prerequisite", "derives", "applies_to", "related"],
                    },
                },
...
def build_concept_graph(
    ...
    db.query(ConceptRelation).filter(
        ConceptRelation.source_id.in_(
            db.query(Concept.id).filter(Concept.textbook_id == textbook_id)
        )
    ).delete(synchronize_session=False)
    db.query(ConceptChunk).filter(
        ConceptChunk.concept_id.in_(
            db.query(Concept.id).filter(Concept.textbook_id == textbook_id)
        )
    ).delete(synchronize_session=False)
    db.query(Concept).filter(Concept.textbook_id == textbook_id).delete(
    ...
    for c in data.get("concepts", []):
        obj = Concept(
            ...
            embedding=embed_text(c["name"] + " " + c.get("description", "")),
        )
        db.add(obj)
        db.flush()
    ...
    for r in data.get("relations", []):
        ...
        if src and tgt:
            db.add(
                ConceptRelation(source_id=src, target_id=tgt, relation_type=r["type"])
            )
...
def search_concepts(
    ...
    qs = db.query(Concept).filter(
        Concept.name.ilike(f"%{q}%") | Concept.description.ilike(f"%{q}%")
    )
    if textbook_id:
        qs = qs.filter(Concept.textbook_id == textbook_id)
    return [
        ConceptResponse(id=r.id, name=r.name, description=r.description)
        for r in qs.limit(10).all()
    ]
...
def get_concept_graph(
    ...
    for rel in rels:
        ...
        if o:
            related[rel.relation_type].append(
                ConceptResponse(id=o.id, name=o.name, description=o.description)
            )
    return ConceptGraphResponse(
        concept=ConceptResponse(id=c.id, name=c.name, description=c.description),
...
def ask(
    ...
    answer, chunks, parsed = answer_query(
        ...
        textbook_ids=payload.textbook_ids,
        group_name=payload.group_name,
    ...
    sources = [
        SourceModel(
            ...
            snippet=best_excerpt(c["content"], payload.prompt),
        )
        for c in chunks
    ]
