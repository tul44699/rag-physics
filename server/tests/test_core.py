...
from services import (
    FLASHCARD_SCHEMA,
    TASK_SCHEMAS,
    _extract_referenced_sources,
    _parse_and_render,
    _render_structured_markdown,
)
...
class TestPrompts:
    ...
    def test_user_prompt_history(self):
        ...
        without_history = build_user_prompt(
            prompt="t", task="qa", context="c", eq_context="", history_text=""
        )
...
class TestRetrieval:
    ...
    def test_rrf_fusion(self):
        shared = "s1"
        a = [
            {"id": shared, "content": "x", "score": 0.9},
            {"id": "a2", "content": "y", "score": 0.8},
        ]
        b = [
            {"id": "b1", "content": "z", "score": 0.9},
            {"id": shared, "content": "x", "score": 0.5},
        ]
...
class TestServices:
    ...
    def test_parse_and_render_flashcards(self):
        parsed, rendered = _parse_and_render(
            '[{"front":"Q?","back":"A","source_n":1}]', "flashcards"
        )
        assert len(parsed) == 1
    ...
    def test_extract_referenced_sources(self):
        chunks = [
            {"id": f"c{i}", "textbook_title": "T", "content": "x", "page_start": i + 1}
            for i in range(3)
        ]
