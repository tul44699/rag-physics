...
export function TextbookProvider({ children }: { children: ReactNode }) {
  ...
  const setChapter = useCallback((chapter: string | null) => setCurrentChapter(chapter), []);

  return (
    <TextbookContext.Provider
      value={{ currentTextbookId, currentPage, currentChapter, setTextbook, setPage, setChapter }}
    >
      {children}
    </TextbookContext.Provider>
  );
