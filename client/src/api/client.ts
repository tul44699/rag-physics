...
export const api = {
  ...
  getTextbookChunks: (id: number, params?: { chapter?: string; page?: number }) => {
  ...
  getProfile: () => request<{ profile: Record<string, unknown> }>('/api/profile'),
  ...
  createEvent: (event: { event_type: string; chapter?: string | null; textbook_id?: number | null; minutes_spent?: number; score?: number | null }) =>
    request('/api/events', {
      method: 'POST',
  ...
  generateFormulaSheet: (params: { textbookIds: number[]; chapter?: string | null; pageStart?: number | null; pageEnd?: number | null }) =>
