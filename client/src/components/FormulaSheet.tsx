...
export default function FormulaSheet() {
	...
	const [sections, setSections] = useState<Record<
		string,
		Record<string, FormulaSheetItem[]>
	> | null>(null);
	const [chapterOrder, setChapterOrder] = useState<string[] | null>(null);
	const [sectionOrder, setSectionOrder] = useState<Record<
		string,
		string[]
	> | null>(null);
	...
	const generate = useCallback(async () => {
		...
		try {
			const fromCh =
				chapterFromIdx >= 0 && chapterFromIdx < chapters.length
					? chapters[chapterFromIdx]
					: null;
			const toCh =
				chapterToIdx >= 0 && chapterToIdx < chapters.length
					? chapters[chapterToIdx]
					: null;
		...
		<div className={styles.page}>
			...
			<div className={styles.controls}>
				<div className={styles.textbookList}>
					...
					{loadingTextbooks && (
						<p className={styles.hint}>Loading textbooks...</p>
					)}
					{textbooks.map((tb) => (
				...
				<div className={styles.label}>
					...
					<div style={{ display: "flex", gap: "0.35rem" }}>
						...
						>
							...
							{chapters.map((ch, i) => (
								<option key={`from-${ch.id}-${i}`} value={i}>
									{ch.title}
								</option>
							))}
						</select>
				...
				<div className={styles.result}>
					{orderedChapters.length === 0 ? (
						<p className={styles.empty}>
							No formulas found for the selected filters.
						</p>
					) : (
						orderedChapters.map((ch) => {
							...
							if (
								!secMap ||
								typeof secMap !== "object" ||
								Object.keys(secMap).length === 0
							)
								return null;
