    ...
    <div className={styles.page}>
      <div className={styles.header}>
        ...
        <p className={styles.subtitle}>
          Upload textbooks via the ingest API, then open one to read and ask the AI assistant questions.
        </p>
        <Link to="/formulasheet" className={styles.formulaLink}>Formula Sheet Generator</Link>
      </div>
      ...
      {error && (
        <div className={styles.error}>
          <span>{error}</span>
          <button onClick={fetchBooks} className={styles.retry}>Retry</button>
        </div>
      )}
      ...
      {!loading && !error && textbooks.length > 0 && (
        <div className={styles.grid}>
          {textbooks.map((tb) => (
            <Link key={tb.id} to={`/textbooks/${tb.id}`} className={styles.card}>
              <h2 className={styles.cardTitle}>{tb.title}</h2>
              {tb.group_name && <span className={styles.badge}>{tb.group_name}</span>}
              <div className={styles.meta}>
                {tb.page_count != null && <span>{tb.page_count} pages</span>}
                {tb.chapter_count != null && <span>{tb.chapter_count} chapters</span>}
              </div>
