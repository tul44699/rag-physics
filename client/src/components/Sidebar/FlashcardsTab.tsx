...
import { useGeneratedContent, type FlashcardDeck, type Flashcard } from '../../context/GeneratedContentContext';
...
export default function FlashcardsTab() {
  ...
  if (decks.length === 0) {
    return <p className={styles.empty}>No flashcards yet. Use the <strong>Chat</strong> tab and select "Flashcards" as the task type to generate some.</p>;
  }
      ...
      {decks.map((deck) => (
        <div key={deck.id} className={styles.deckItem}>
          <button className={styles.deckHeader} onClick={() => toggle(deck.id)}>
            <span className={styles.chevron}>{expanded.has(deck.id) ? '▾' : '▸'}</span>
            <span>{deck.cards.length} cards</span>
            <span className={styles.time}>{new Date(deck.timestamp).toLocaleTimeString()}</span>
          </button>
