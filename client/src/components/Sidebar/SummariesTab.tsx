...
export default function SummariesTab() {
  ...
  if (!currentTextbookId) return <p className={styles.empty}>Open a textbook to view chapter summaries.</p>;
  const summaries = getForTextbook(currentTextbookId).chapterSummaries;
  if (summaries.length === 0) return <p className={styles.empty}>No summaries yet. Use the <strong>Chat</strong> tab and select "Chapter Summary" to generate one.</p>;
