...
export default function FormulasTab() {
  ...
  useEffect(() => {
    ...
    api.getTextbook(currentTextbookId).then(tb => setChapters(tb.chapters ?? [])).catch(() => setChapters([]));
  }, [currentTextbookId]);

  if (!currentTextbookId) return <p className={styles.empty}>Open a textbook to view formulas.</p>;
  ...
  const generate = async () => {
    ...
    try {
      ...
      setFormulaSheet(currentTextbookId, { sections: data.sections ?? {}, chapterOrder: data.chapter_order ?? [], section_order: data.section_order ?? {} });
    } catch (e) {
  ...
  return (
    <div className={styles.container}>
      <div className={styles.controls}>
        <select value={chapterFromIdx} onChange={e => setChapterFromIdx(Number(e.target.value))}>
          <option value={-1}>From...</option>
          {chapters.map((ch, i) => (<option key={ch.id} value={i}>{ch.title}</option>))}
        </select>
        <select value={chapterToIdx} onChange={e => setChapterToIdx(Number(e.target.value))}>
          <option value={-1}>To...</option>
          {chapters.map((ch, i) => (<option key={ch.id} value={i}>{ch.title}</option>))}
        </select>
        <button className={styles.btn} onClick={generate} disabled={loading}>{loading ? '...' : 'Generate'}</button>
      </div>
      {error && <p className={styles.error}>{error}</p>}
      {sheet ? (
        sheet.chapterOrder.length === 0 ? <p className={styles.empty}>No formulas found.</p> : (
                ...
                {secOrder.map(sec => {
                  ...
                  return (
                    <div key={sec} className={styles.section}>
                      {sec !== 'General' && <h4 className={styles.sectionTitle}>{sec}</h4>}
                      {eqs.map((eq, i) => <FormulaItem key={i} eq={eq} />)}
                    </div>
                  );
                })}
              </div>
      ...
      ) : <p className={styles.empty}>Select a chapter range and click Generate.</p>}
    </div>
  );
}
  ...
  return (
    <div className={styles.item}>
      <span className={styles.itemText}>
        {html ? <span dangerouslySetInnerHTML={{ __html: html }} /> : <code>{eq.plain_text}</code>}
      </span>
      {eq.page_start != null && <span className={styles.pageNum}>p.{eq.page_start}</span>}
    </div>
  );
}
