...
import { useGeneratedContent, type GeneratedItem } from '../../context/GeneratedContentContext';
...
export default function StudyGuidesTab() {
  ...
  if (!currentTextbookId) return <p className={styles.empty}>Open a textbook to view study guides.</p>;
  const guides = getForTextbook(currentTextbookId).studyGuides;
  if (guides.length === 0) return <p className={styles.empty}>No study guides yet. Use the <strong>Chat</strong> tab and select "Study Guide" to generate one.</p>;
      ...
      {items.map(item => (
        <div key={item.id} className={styles.deckItem}>
          <button className={styles.deckHeader} onClick={() => toggle(item.id)}>
            <span className={styles.chevron}>{expanded.has(item.id) ? '▾' : '▸'}</span>
