  ...
  return (
    <div className={styles.container}>
      <div className={styles.zoomBar}>
        <button onClick={() => setZoom((z) => Math.max(0.5, z - 0.2))} className={styles.zoomBtn}>-</button>
        <span className={styles.zoomLabel}>{Math.round(zoom * 100)}%</span>
        <button onClick={() => setZoom((z) => Math.min(2.5, z + 0.2))} className={styles.zoomBtn}>+</button>
      </div>
      <Document
        file={file}
